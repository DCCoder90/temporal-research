# temporal-lens — User Guide

**temporal-lens** turns a `.pcap` capture from a running Temporal cluster into interactive diagrams, statistics, and a SQL-queryable dataset. It works as a native desktop GUI or a headless CLI.
...
`temporal-lens` reads `config.json` at startup to know how to resolve container IPs to names and how to label protocol ports. **The file must be present** — the application will refuse to start if it is missing.
...
2. **`~/.config/temporal-lens/`** — the preferred location for the GUI, since the binary lives inside a `.app` bundle on macOS.
...
5. Restart `temporal-lens`

A default `config.json` for the standard `temporal-lens` Docker Compose setup is included in every release archive. It is a good starting point for customisation.
...
Searched:
  /usr/local/bin
  /Users/yourname/.config/temporal-lens
...
temporal-lens capture.pcap --only-host hello-world-worker

# Include multiple hosts
temporal-lens capture.pcap --only-host hello-world-worker,hello-world-starter

# Exclude Wireshark and the Temporal UI from diagrams
temporal-lens capture.pcap --exclude-host wireshark,temporal-ui

# Combine protocol and host filters (ANDed)
temporal-lens capture.pcap --only grpc --only-host hello-world-worker
```
...
```bash
# Pipe to jq
temporal-lens capture.pcap --json --quiet | jq '.grpc_calls | map(.method) | unique'

# Save to file
temporal-lens capture.pcap --json --quiet > analysis.json
```
...
The application requires `config.json` to be present before it can start. It searches in the binary's directory first, then `~/.config/temporal-lens/`. See [Configuration files](#configuration-files) for full details.
